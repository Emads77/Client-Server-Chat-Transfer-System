...
class SingleUserTests {
    ...
    void tc54EmptyJsonMessageReturnsError() throws JsonProcessingException {
        receiveLineWithTimeout(in); // Ready message
        out.println("ENTER "); // Malformed JSON
        out.flush();
        String serverResponse = receiveLineWithTimeout(in);
        ParseError parseError = Utils.messageToObject(serverResponse);
    ...
    @Test
    void tcPongWithoutPingReturnsPongError() throws JsonProcessingException {
        receiveLineWithTimeout(in); // Ready message
        out.println(Utils.objectToMessage(new Enter("emad"))); // Login
        out.flush();
        receiveLineWithTimeout(in); // OK
        out.println(Utils.objectToMessage(new Pong())); // Send PONG without PING
        out.flush();
        String serverResponse = receiveLineWithTimeout(in);
